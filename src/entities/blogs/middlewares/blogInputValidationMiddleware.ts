...
const additionalWebsiteUrlRules: ((chain: ValidationChain) => ValidationChain)[] = [
  (chain) =>
    chain
      .matches(/^(https?:\/\/)([a-zA-Z0-9_-]+\.)+[a-zA-Z0-9_-]+(\/[a-zA-Z0-9._-]*)*\/?$/)
      .withMessage('wrong website url format'),
];
...
export const deleteBlogValidators = [authMiddleware, validateUrlParamId];

export const updateBlogBodyValidators = [...deleteBlogValidators, ...addBlogBodyValidators];
