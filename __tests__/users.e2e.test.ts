...
describe('/users', () => {
    ...
    it('should return both created users in case when searchLoginTerm ' +
        'is matched with first user login and searchEmailTerm is matched with second user email ', async ()=>{
        const res = await req
            .get(PATHS.USERS)
            .query({searchLoginTerm:newUser.login.slice(1)})
    ...
    it('should remove user by provided id',async ()=>{
        ...
        const newUserId = await req.get(PATHS.USERS)
            .set('Authorization', 'Basic YWRtaW46cXdlcnR5')
            .expect(200)
